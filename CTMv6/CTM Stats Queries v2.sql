...
drop view if exists ctm_stats_daily_2;
create view ctm_stats_daily_2 as
...
select 26 seq_no, tran_dt, '' stats from ctm_stats_report where tran_type='ukactive' group by tran_dt union
select 27 seq_no, tran_dt, 'Unique Active Users' stats from ctm_stats_report where tran_type='ukactive' group by tran_dt union
select 28, tran_dt, concat(carrier, ': ', total,'
   Pre: ', pre, '
   Post: ', post) stats from ctm_stats_report where tran_type = 'ukactive' and carrier <> 'others' union
select 29 seq_no, tran_dt, '' stats from ctm_stats_report where tran_type='p2p' group by tran_dt union
select 30, tran_dt, concat('Active PC Accounts', ': ', pre) stats from ctm_stats_report where tran_type = 'ukactive' and carrier = 'others' union
select 31 seq_no, tran_dt, concat ('P2P Messages:', pre) stats from ctm_stats_report where tran_type='p2p' group by tran_dt
;
