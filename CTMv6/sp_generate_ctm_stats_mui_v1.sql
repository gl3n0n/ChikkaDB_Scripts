...
begin
      ...
      REPEAT
         ...
         IF not done_p THEN
            ...
            SET @vSql = concat('select count(distinct chikkaid) INTO @UkActive from (',
                               ...
                               'union ', 
                               'select tx_from chikkaid from mui_ph_globe_1.bridge_out where datein=''', vDatein, ''' and status = 2 and tx_type = ''messaging'' and tx_from REGEXP ''^', vPattern, '$'') tab1');
            PREPARE stmt FROM @vSql;
            EXECUTE stmt;
            DEALLOCATE PREPARE stmt;
   ...
   SET @vSql = concat('select count(distinct chikkaid) INTO @TotalUkActiveM from (',
                      'select tx_to chikkaid from mui_ph_globe_1.bridge_in where datein=''', vDatein, ''' and status = 2 and tx_type = ''chat'' ',
                      'union ', 
                      'select tx_from chikkaid from mui_ph_globe_1.bridge_out where datein=''', vDatein, ''' and status = 2 and tx_type = ''messaging'') tab1');
   PREPARE stmt FROM @vSql;
   EXECUTE stmt;
   DEALLOCATE PREPARE stmt;
   ...
   BEGIN
      ...
      REPEAT
         ...
         IF not done_p THEN
            ...
            SET @vSql = concat('select count(distinct tx_to) INTO @UkMt from mui_ph_smart_1.bridge_in where datein=''', vDatein, ''' and status = 2 and tx_type = ''chat'' and tx_to REGEXP ''^', vPattern, '$''');
            PREPARE stmt FROM @vSql;
            EXECUTE stmt;
            DEALLOCATE PREPARE stmt;

            SET @vSql = concat('select count(distinct chikkaid) INTO @UkActive from (',
                               'select tx_to chikkaid from mui_ph_smart_1.bridge_in where datein=''', vDatein, ''' and status = 2 and tx_type = ''chat'' and tx_to REGEXP ''^', vPattern, '$'' ',
                               'union ', 
                               'select tx_from chikkaid from mui_ph_smart_1.bridge_out where datein=''', vDatein, ''' and status = 2 and tx_type = ''messaging'' and tx_from REGEXP ''^', vPattern, '$'') tab1');
            PREPARE stmt FROM @vSql;
            EXECUTE stmt;
            DEALLOCATE PREPARE stmt;
   ...
   -- GET TOTAL MOBILE ACTIVE USERS
   SET @vSql = concat('select count(distinct chikkaid) INTO @TotalUkActiveM from (',
                      'select tx_to chikkaid from mui_ph_smart_1.bridge_in where datein=''', vDatein, ''' and status = 2 and tx_type = ''chat'' ',
                      'union ', 
                      'select tx_from chikkaid from mui_ph_smart_1.bridge_out where datein=''', vDatein, ''' and status = 2 and tx_type = ''messaging'') tab1');
   PREPARE stmt FROM @vSql;
   EXECUTE stmt;
   DEALLOCATE PREPARE stmt;
   ...
   BEGIN
      ...
      REPEAT
         ...
         IF not done_p THEN
            ...
            SET @vSql = concat('select count(distinct tx_to) INTO @UkMt from mui_ph_sun_1.bridge_in where datein=''', vDatein, ''' and status = 2 and tx_type = ''chat'' and tx_to REGEXP ''^', vPattern, '$''');
            PREPARE stmt FROM @vSql;
            EXECUTE stmt;
            DEALLOCATE PREPARE stmt;

            SET @vSql = concat('select count(distinct chikkaid) INTO @UkActive from (',
                               'select tx_to chikkaid from mui_ph_sun_1.bridge_in where datein=''', vDatein, ''' and status = 2 and tx_type = ''chat'' and tx_to REGEXP ''^', vPattern, '$'' ',
                               'union ', 
                               'select tx_from chikkaid from mui_ph_sun_1.bridge_out where datein=''', vDatein, ''' and status = 2 and tx_type = ''messaging'' and tx_from REGEXP ''^', vPattern, '$'') tab1');
            PREPARE stmt FROM @vSql;
            EXECUTE stmt;
            DEALLOCATE PREPARE stmt;
   ...
   -- GET TOTAL MOBILE ACTIVE USERS
   SET @vSql = concat('select count(distinct chikkaid) INTO @TotalUkActiveM from (',
                      'select tx_to chikkaid from mui_ph_sun_1.bridge_in where datein=''', vDatein, ''' and status = 2 and tx_type = ''chat'' ',
                      'union ', 
                      'select tx_from chikkaid from mui_ph_sun_1.bridge_out where datein=''', vDatein, ''' and status = 2 and tx_type = ''messaging'') tab1');
   PREPARE stmt FROM @vSql;
   EXECUTE stmt;
   DEALLOCATE PREPARE stmt;
   ...
   SET @vSql = concat('select count(distinct chikkaid) INTO @TotalUkActiveP from (',
                      'select tx_from chikkaid from mui_ph_sun_1.bridge_in where datein=''', vDatein, ''' and status = 2 and tx_type = ''chat'' and tx_from like ''00%'' group by tx_from ',
                      'union ', 
                      'select tx_to chikkaid from mui_ph_sun_1.bridge_out where datein=''', vDatein, ''' and status = 2 and tx_type = ''messaging'' and tx_to like ''00%'' group by tx_to ',
                      'union ', 
                      'select tx_from chikkaid from mui_ph_globe_1.bridge_in where datein=''', vDatein, ''' and status = 2 and tx_type = ''chat'' and tx_from like ''00%'' group by tx_from ',
                      'union ', 
                      'select tx_to chikkaid from mui_ph_globe_1.bridge_out where datein=''', vDatein, ''' and status = 2 and tx_type = ''messaging'' and tx_to like ''00%'' group by tx_to ',
                      'union ', 
                      'select tx_from chikkaid from mui_ph_smart_1.bridge_in where datein=''', vDatein, ''' and status = 2 and tx_type = ''chat'' and tx_from like ''00%'' group by tx_from ',
                      'union ', 
                      'select tx_to chikkaid from mui_ph_smart_1.bridge_out where datein=''', vDatein, ''' and status = 2 and tx_type = ''messaging'' and tx_to like ''00%'' group by tx_to ',
                      ') as tab1');
   PREPARE stmt FROM @vSql;
   EXECUTE stmt;
   DEALLOCATE PREPARE stmt;
