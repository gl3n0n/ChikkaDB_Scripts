create table ctm_stats_dtl (
   ...
   type  enum('reg', 'hits', 'mo','mt','ukmo', 'ukmt') not null,
   app    varchar(20) not null,
   source varchar(20) not null,
   post  int(8) default 0 not null,
   pre   int(8) default 0 not null,
   oth   int(8) default 0 not null,
   primary key (tran_dt, type, app, source)
);


insert into ctm_stats_dtl (tran_dt, type, source, post, pre)
select reg_date, 'reg', msisdn_carrier
