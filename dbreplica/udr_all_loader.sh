...
#    echo $db_conn,$db_host
#echo "select CONCAT("for a in `ls *.bz2` ; do""
   
# echo "Processing TopTalkers Started for $a" >> $var_log/$var_cur.sftp.log
# echo $a
#   file_cnt=`wc -l $a.txt | awk '{print $1}'`
#   file_cnt2=$((file_cnt - 1))
#  echo $file_cnt 
#  echo $a.txt 
#  flcnt=`echo $a.txt | sed 's/\/mnt\/paywall_dmp\/dmp\/udr\///g'`
#  flcnt2=`echo $a.txt | sed 's/\/mnt\/paywall_dmp\/dmp\/udr\///g'| cut -d '_' -f 3-5`
#  `$db_conn $db_opt "INSERT INTO tmp_udr_log_uploads(logfile,rawfile_cnt )VALUES('$flcnt',$file_cnt2)"` 
#rm -f $a
#  echo "DB loading  part2" >> $var_log/$var_cur.sftp.log
#  `$db_conn $db_opt "LOAD DATA LOCAL INFILE '$a.txt'  IGNORE  INTO TABLE archive_powerapp_flu.powerapp_udr_log  FIELDS TERMINATED by ',' IGNORE 1 LINES (tx_date,start_tm,end_tm,phone,ip_addr,source,service,b_usage,device) SET gen_date='$flcnt2'"`
# rm -f $a.txt
}
