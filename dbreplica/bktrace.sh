...
if [[ $# -eq 2 ]] ; then
   if [[ $1 =~ ^$date_format_1$ ]] && [[ $2 =~ ^$date_format_2$ ]] ; then
    ...
    #    cleanup_date=`date -d "$tab_date 1 day ago" +%Y-%m-%d`
        cleanup_date=`date -d "$proc_date $ctr day" +%Y-%m-%d`
        #var_date proc_date p_trandate of bkdate_new_powerapp_udr_log.sh   
#        /home/mysql/scripts/bkdate_new_powerapp_udr_log.sh $filedate $tab_date $cleanup_date > $var_log/$proc_date".reproc_udr.log" 2>&1 & -- facebook
#       sh -x /home/mysql/scripts/bkdate_new_powerapp_udr_log.sh.all $filedate $tab_date $cleanup_date > $var_log/$proc_date".reproc_udr.log" 2>&1 &
        sh -x /home/mysql/scripts/bktrace_powerapp_udr_log.sh $filedate $tab_date $cleanup_date > $var_log/$proc_date".reproc_udr.log" 2>&1 &
        sleep 60m 
        ctr=$(( $ctr + 1 ))
