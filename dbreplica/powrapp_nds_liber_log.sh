...
file_date=`date +%Y%m%d -d '1 day ago'`
...
#   logger "PULL data process from sandvine started"
  ...
  #         /home/mysql/scripts/expect/exp.nds_logs_getter.exp.trace $var_usr $var_pn $line
          /home/mysql/scripts/expect/exp.nds_logs_getter.exp.cur $var_usr $var_pn $line
          let COUNTER-=1
...
load_db_dmp_() {
  ...
  `$db_conn $db_opt "LOAD DATA LOCAL INFILE '$var_dmp_in/$1.csv'  IGNORE  INTO TABLE archive_powerapp_flu.powerapp_nds_log FIELDS TERMINATED by ',' (phone,service,transmit,received) SET tx_date='$1'"`
...
#        mv $var_dmp_in/"$var_date".csv.gz  $var_dmp_out
        ...
        gzip -v9 $var_dmp_in/"$var_date".csv 
        mv $var_dmp_in/"$var_date".csv.gz  $var_dmp_out
        lib_mins_date=`date +%Y-%m-%d -d '2 day ago'`
        nds_lib_date=`date +%Y-%m-%d -d '1 day ago'`
        tnt_usage_date=`date +%Y%m%d -d '1 day ago'`
        `$db_conn $db_opt " SELECT 'phone','transaction_date','tot_fb_usage','total_usage_mb' UNION  select a.phone, b.tx_date, round(sum(IF(b.service='FacebookService',b.transmit+b.received,0))/1000000,2) total_fb_usage, round(sum(b.transmit+b.received)/1000000,2) total_usage_mb  into outfile '/mnt/paywall_dmp/liberation_tnt2/Liberation_TNT_usage_$tnt_usage_date.csv' fields terminated by ',' lines terminated by '\n'  from  tmp_liberation_mins a, powerapp_nds_log b where a.phone = b.phone and   a.datein < 'lib_mins_date' and   b.tx_date = '$nds_lib_date' and  a.brand = 'TNT' group by 1,2;"` 
`$db_conn $db_opt " SELECT 'phone','transaction_date','tot_fb_usage','total_usage_mb' UNION  select a.phone, b.tx_date, round(sum(IF(b.service='FacebookService',b.transmit+b.received,0))/1000000,2) total_fb_usage, round(sum(b.transmit+b.received)/1000000,2) total_usage_mb  into outfile '/mnt/paywall_dmp/liberation_tnt2/Liberation_POSTPD_usage_$tnt_usage_date.csv' fields terminated by ',' lines terminated by '\n'  from  tmp_liberation_mins a, powerapp_nds_log b where a.phone = b.phone and   a.datein < 'lib_mins_date' and   b.tx_date = '$nds_lib_date' and  a.brand = 'POSTPD' group by 1,2;"`
`$db_conn $db_opt " SELECT 'phone','transaction_date','tot_fb_usage','total_usage_mb' UNION  select a.phone, b.tx_date, round(sum(IF(b.service='FacebookService',b.transmit+b.received,0))/1000000,2) total_fb_usage, round(sum(b.transmit+b.received)/1000000,2) total_usage_mb  into outfile '/mnt/paywall_dmp/liberation_tnt2/Liberation_BUDDY_usage_$tnt_usage_date.csv' fields terminated by ',' lines terminated by '\n'  from  tmp_liberation_mins a, powerapp_nds_log b where a.phone = b.phone and   a.datein < 'lib_mins_date' and   b.tx_date = '$nds_lib_date' and  a.brand = 'BUDDY' group by 1,2;"`
        `$db_conn $db_opt " TRUNCATE TABLE powerapp_nds_log"`     
