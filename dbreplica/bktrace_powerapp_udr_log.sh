...
#           done " > $1"/ftp_"$COUNTER"/"$COUNTER"_bzcatp.sh"
 ...
 #find /mnt/paywall_dmp/dmp/udr -name "*bz2-1-txt" -type f | xargs -r ls  | wc -l
             ...
             #    echo "\`$db_conn $db_opt \"INSERT INTO test_1.loaded_udr_files VALUES('$line'); SET BULK_INSERT_BUFFER_SIZE = 20*1024*1024; LOAD DATA LOCAL INFILE '$line' IGNORE  INTO TABLE test_1.udr_$cnt2  FIELDS TERMINATED by ',' (tx_date,phone,source,service,rx,tx,b_usage)\"\`" >> $4/$cnt2-$var_date.sql 
                  echo "\`$db_conn $db_opt \"INSERT INTO test_1.loaded_udr_files VALUES('$line'); SET BULK_INSERT_BUFFER_SIZE = 20*1024*1024; LOAD DATA LOCAL INFILE '$line' IGNORE  INTO TABLE test_1.udr_$cnt2  FIELDS TERMINATED by ',' (tx_date,strt_time,phone,source,service,rx,tx,b_usage) SET filename='$line2' \"\`" >> $4/$cnt2-$var_date.sql
...
cleanup_proc() {
    if [ $5 = 1 ] ; then
       ...
       until [ $dbcnt -eq 51 ] ;
       do
           `$db_conn $db_opt "ALTER TABLE test_1.udr_$dbcnt drop partition p_$db_partition_dt;"` 
#             `$db_conn $db_opt "truncate table test_1.udr_file_1;truncate table test_1.udr_file_2"`
             ((dbcnt++))
       done
 ...
 # etl_proc $var_dmp/udr $udrproc_log_dir $var_date $tmpload_scripts_dir 2 1 # LOADING
  ...
  if [ "$DLudr" -eq "$UdrDLList" ] ; then
   ...
   ##  cleanup_proc $var_dmp/udr $udrproc_log_dir $tmpload_scripts_dir $var_date 4 3
     sleep 5m
     `$db_conn2 $db_opt "call sp_generate_udr_toptalker_bktrace('$p_trandate')"` 
  else
