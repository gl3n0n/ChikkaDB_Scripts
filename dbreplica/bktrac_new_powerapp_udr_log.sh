...
#var_date=`date +%Y_%m_%d -d '17 day ago'`
#proc_date=`date +%Y-%m-%d -d '17 day ago'`
#p_trandate=`date +%Y-%m-%d -d '18 day ago'`
var_date=$1
proc_date=$2
p_trandate=$3
#db_partition_dt=`date +%Y%m%d -d '5 day ago'`
#db_partition_dt=`date +%Y%m%d -d '6 day ago'`
db_partition_dt=`date +%Y%m%d -d '19 day ago'`
var_cur=`date +%Y%m%d`
# %k     hour ( 0..23)
...
db_prt=3307
db_host="127.0.0.1"
db_conn="mysql -ustats -pstats test_1 -h$db_host -P$db_prt"
...
#           echo "cd "$1"/ftp_"$COUNTER"  COMMENTED BY ALLAN 2015-05-21
#           for a in \`ls *$var_date*.bz2*\`
#           do
...
#           done " > $1"/ftp_"$COUNTER"/"$COUNTER"_bzcatp.sh"
           ...
           for a in \`ls *$var_date*.bz2*\`
           do
           bzcat \$a | awk -F',' '{printf \$4\"+\"\$5\"+\"\$7\"+\"\$14\"+\"\$15\"+\"\$16\"+\"\$17\"+\"\$18 \"\\n\"}'  | awk -F'+' '{printf substr(\$1,0,10)\",\"substr(\$1,12,8)\",\"substr(\$3,12,8)\",\"\$5\",\"\$6\",\"\$7\",\"\$8\",\"\$9\",\"\$10\"\\n\"}' | awk -F',' '\$7!~/0/{print \$0}' | grep -i '639989659677\|639989670051\|639989670081\|639989666457\|639989666412\|639989666359\|639989666395\|639989659685\|639989659716\|639989659771\|639989687584\|639989687585\|639989687586\|639989687587\|639989687563\|639989687552\|639989687553\|639989687554\|639989687556\|639989687558'  | grep -v ",,,,,," | sed '1d' > \$a-txt
           done " > $1"/ftp_"$COUNTER"/"$COUNTER"_bzcatp.sh"
 ...
 #find /mnt/paywall_dmp/dmp/udr -name "*bz2-1-txt" -type f | xargs -r ls  | wc -l
             ...
             #    echo "\`$db_conn $db_opt \"INSERT INTO test_1.loaded_udr_files VALUES('$line'); SET BULK_INSERT_BUFFER_SIZE = 20*1024*1024; LOAD DATA LOCAL INFILE '$line' IGNORE  INTO TABLE test_1.udr_$cnt2  FIELDS TERMINATED by ',' (tx_date,phone,source,service,rx,tx,b_usage)\"\`" >> $4/$cnt2-$var_date.sql 
                  echo "\`$db_conn $db_opt \"INSERT INTO test_1.loaded_udr_files VALUES('$line'); SET BULK_INSERT_BUFFER_SIZE = 20*1024*1024; LOAD DATA LOCAL INFILE '$line' IGNORE  INTO TABLE test_1.udr_$cnt2  FIELDS TERMINATED by ',' (tx_date,strt_time,end_time,phone,source,service,rx,tx,b_usage) SET filename='$line2' \"\`" >> $4/$cnt2-$var_date.sql
...
cleanup_proc() {
    if [ $5 = 1 ] ; then
       ...
       logger "Test DB Cleanup"
             `$db_conn $db_opt "call test_1.sp_generate_facebook_usage_clear('$p_trandate')"`
       ...
       find $1 -type d -name "ftp_*" -exec rm -r {} \;
         ...
         if [ $6 = 1 ] ; then
             ...
             logger "Bzip Cleanup Started"
             mkdir /mnt/paywall_dmp/compress_udr_txt/"$var_date"
             find $1  -name "SANDVINE*-txt"  -exec gzip -v9 {} \;
             #find $1 -name "*.gz" | xargs mv -t /mnt/paywall_dmp/compress_udr_txt/"$var_date"
             #sleep 1
             ind $1  -name "SANDVINE*bz2*" | cut -d"-" -f1,2 | xargs -r rm -f # xargs -r -P2 ls >> /tmp/ajz.tzt 
             logger "Bzip2 Cleanup Complete"
         fi 
          ...
          if [ $6 =  1 ]  ; then 
               until [ $cnt2 -eq 51 ] ;
              do
                 echo "\`$db_conn $db_opt \" ALTER TABLE test_1.udr_$cnt2 disable keys;)\"\`" >> $3/$cnt2-$4.disablekey.sql
              ...
              until [ $cnt2 -eq 51 ] ;
              do
                 echo "\`$db_conn $db_opt \" ALTER TABLE test_1.udr_$cnt2 enable keys;)\"\`" >> $3/$cnt2-$4.enablekey.sql
              ...
              until [ $cnt2 -eq 51 ] ;
              do
                 echo "\`$db_conn $db_opt \" DELETE FROM test_1.udr_$cnt2 WHERE tx_date='0000-00-00';)\"\`" >> $3/$cnt2-$4.clean0.sql
...
#     sleep 5m
 ...
 #    sleep 5m
  #   `$db_conn2 $db_opt "call sp_generate_udr_toptalker('$p_trandate')"` 
    `$db_conn $db_opt "call sp_generate_uber_usage('$proc_date')"`
     cleanup_proc $var_dmp/udr $udrproc_log_dir $tmpload_scripts_dir $var_date 3 7
         
  else
     echo "problems pa din"
    `$db_conn $db_opt "TRUNCATE test_1.udr_file_1;TRUNCATE  test_1.udr_file_2;"`

    `$db_conn $db_opt "LOAD DATA INFILE '$udrproc_log_dir/b_"$var_date"_udrfile_1' INTO TABLE test_1.udr_file_1 LINES TERMINATED by '\r\n';LOAD DATA INFILE '$udrproc_log_dir/sorted_"$var_date"_udrlist_2' INTO TABLE test_1.udr_file_2 FIELDS TERMINATED  BY '\n';  select CONCAT('$rem_udr_path', a.udr_file) FROM  udr_file_1 a LEFT JOIN udr_file_2 b on a.udr_file=b.udr_file WHERE b.udr_file is NULL INTO OUTFILE '$udrproc_log_dir/"$var_date"_udr_reprocess.txt'"`
    sleep 1
