...
load_dmp_() {
  echo "reporcess" >> $var_log/$var_cur.sftp.log
  echo $a
  logger "udr_reprocess.sh: reprocessing file $a"
  ...
  rm -f $a
  echo "reporcess db part3" >> $var_log/$var_cur.sftp.log

  logger "udr_reprocess.sh: loading file $a" 
