drop procedure sp_smartservice_dailyrep;
delimiter //
...
BEGIN
    ...
    BEGIN
        ...
        SET @vRepDailyOtherSQL = CONCAT("SELECT 'Date','Unique Users','Hits','Buys','Buys via App','Buys via Web',
                                                'Buys via SMS','Active MINs','Monthly Unique Buyers','Monthly Active MINs','No. Insuffbal','Unique Insuffbal MINs',
                                                'No. Subs Not Allowed','Unique Subs Not Allowed','No. Invalid Phones','Unique Invalid Phones'
                                         UNION ALL
                                         SELECT tx_date,
                                                MAX(IF(uniq>=0,uniq,0)) as  number_of_uniq_hits,
        ...
        SET vRepDailyOtherRep = CONCAT('/mnt/logs/app_log/daily_hits_per_plan.',@rep_date,'.txt');
        SET @vRepDailyOtherSQL = CONCAT("SELECT 'Plan','Unique Users','Buys','Hits'
                                         UNION ALL select * from (
                                         SELECT plan, uniq, no_buys, hits
                                         FROM stats_smartservice_daily_per_plan
                                         WHERE tx_date = '",@rep_date,"'                                                 
                                         ORDER BY plan) t
                                         INTO OUTFILE '",vRepDailyOtherRep,"'
