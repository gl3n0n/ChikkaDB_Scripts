...
delimiter //
CREATE PROCEDURE sp_generate_bcast_mins(p_startdt date, p_bcastdt date, p_plan_bud varchar(20), p_plan_tnt varchar(20), p_limit int)
BEGIN
   set @vWhere = '';
   set @nCtr  = 0;
   ...
   SET @vSql = concat('insert into tmp_plan_users select a.phone, a.brand, ''POWERAPP'', ''', p_bcastdt, ''', ''powerapp'' from powerapp_users_apn a, tmp_chikka_apn b ',
                      'where a.phone = b.phone ', @vWherePwr, ' limit 500000');
   select @vSql;
   PREPARE stmt FROM @vSql;
   EXECUTE stmt;
   DEALLOCATE PREPARE stmt;
