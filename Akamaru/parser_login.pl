...
my $qry_globe_cnt_postpaid = "SELECT COUNT(DISTINCT(mobile_number)) FROM `oist_log`.`oist_".$event_type."_".$tbl_date."` WHERE mobile_number REGEXP '^".$globe_postpaid[0]."\$' AND status='true'";
$sth_globe_cnt_postpaid = $dbh_oist->prepare($qry_globe_cnt_postpaid);
$sth_globe_cnt_postpaid->execute();

my $sth_globe_cnt_prepaid="";
my $qry_globe_cnt_prepaid = "SELECT COUNT(DISTINCT(mobile_number)) FROM `oist_log`.`oist_".$event_type."_".$tbl_date."` WHERE mobile_number REGEXP '^".$globe_prepaid[0]."\$' AND status='true'";
$sth_globe_cnt_prepaid = $dbh_oist->prepare($qry_globe_cnt_prepaid);
$sth_globe_cnt_prepaid->execute();

my $sth_globe_cnt_tm="";
my $qry_globe_cnt_tm = "SELECT COUNT(DISTINCT(mobile_number)) FROM `oist_log`.`oist_".$event_type."_".$tbl_date."` WHERE mobile_number REGEXP '^".$globe_tm[0]."\$' AND status='true'";
...
my $sth_qry_insert;
my $qry_insert = "REPLACE INTO oist_globe_daily_login VALUES ('$current_date',0,$globe_cnt_postpaid[0],$globe_cnt_prepaid[0],$globe_cnt_tm[0]);";
