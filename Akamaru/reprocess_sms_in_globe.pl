...
my $charging = common::chargingOperator('GLOBE');
#
...
#my $qry_globe_all_sms = "SELECT COUNT(1) FROM sms_in WHERE datein ='$current_date' AND  gsm_num REGEXP '^".$globe_all_pattern[0]."\$'";
my $qry_globe_all_sms = "SELECT COUNT(1) FROM sms_in WHERE datein ='$current_date' AND  access_code LIKE '2371%'";
...
print "\n";
print $qry_globe_postpaid_unique;
print "\n";
print $qry_globe_postpaid_unique;
print "\n";
print $qry_globe_tm_unique;
...
my $sth_qry_insert  = "";
my $qry_insert = "REPLACE INTO oist_stat_globe (datein,sms_revenue,sms_hits,unique_charged_mins,unique_charged_post,unique_charged_pre,unique_charged_tm) 
...
my $strUpdateTotalRevenue = "UPDATE oist_stat_globe SET total_revenue=sms_revenue+voice_revenue WHERE datein ='$current_date'";
