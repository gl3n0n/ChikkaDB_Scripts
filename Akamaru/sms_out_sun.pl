...
my $qry_globe_all_sms = "SELECT COUNT(1) FROM sms_out WHERE datesent ='$current_date' AND  access_code LIKE '2009%'";
...
my $sth_qry_insert  = "";
my $qry_insert = "REPLACE INTO oist_stat_sun_out (datein,sms_revenue,sms_hits,unique_charged_mins,unique_charged_post,unique_charged_pre) VALUES ('$current_date',$globe_cnt_all_sms[0]*$charging,$globe_cnt_all_sms[0],$globe_cnt_all_unique[0],$globe_cnt_postpaid_unique[0],$globe_cnt_all_unique[0]-$globe_cnt_postpaid_unique[0])";
...
my $strUpdateTotalRevenue = "UPDATE oist_stat_sun_out SET total_revenue=sms_revenue+voice_revenue WHERE datein ='$current_date'";
