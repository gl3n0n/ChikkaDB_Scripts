...
my $qry_globe_all_sms = "SELECT COUNT(1) FROM sms_in WHERE datein ='$current_date' AND  access_code LIKE '277%'";
...
my $sth_qry_insert  = "";
my $qry_insert = "REPLACE INTO oist_stat_smart (datein,sms_revenue,sms_hits,unique_charged_mins,unique_charged_post,unique_charged_pre,unique_charged_tnt) 
...
my $strUpdateTotalRevenue = "UPDATE oist_stat_smart SET total_revenue=sms_revenue+voice_revenue WHERE datein ='$current_date'";
