...
$strSQLhi10 = "select DATE_FORMAT(tx_date,'%m/%d/%Y'),
                      phone, transmitted, received, transmitted+received total_usage
                from  tmp_liberation_usage
                where tx_date = '".$current_day."' order by tx_date";
...
$worksheet->merge_range('B2:E2', 'LIBERATION USAGE', $formatC);
...
$worksheet->write($row+$i, $col+1, 'PHONE', $format);
$worksheet->write($row+$i, $col+2, 'TRANSMITTED', $format);
$worksheet->write($row+$i, $col+3, 'RECEIVED', $format);
$worksheet->write($row+$i, $col+4, 'TOTAL', $format);
...
while (@rowRst = $sth_hi_10->fetchrow()) {
   ...
   $worksheet->write($row+$i, $col+4,  $rowRst[4], $formatT);
}                      
