...
my $qry_globe_all_sms = "SELECT COUNT(1) FROM sms_out WHERE datesent ='$current_date' AND  access_code LIKE '2371%'";
...
my $sth_qry_insert  = "";
my $qry_insert = "REPLACE INTO oist_stat_globe_out (datein,sms_revenue,sms_hits,unique_charged_mins,unique_charged_post,unique_charged_pre,unique_charged_tm) 
...
my $strUpdateTotalRevenue = "UPDATE oist_stat_globe_out SET total_revenue=sms_revenue+voice_revenue WHERE datein ='$current_date'";
