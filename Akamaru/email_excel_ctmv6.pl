...
$worksheet[0]->set_column(91,91,1);
$row = 2;
$col = 0;
$i=1;
#GLOBE
...
$i=1;
$strSQLCTMv6 = "
select max(if((carrier = 'SMART'),`total`,0)) AS `smart`,
       max(if((carrier = 'GLOBE'),`total`,0)) AS `globe`,
...
while (@rowRst = $sth_ctm_v6->fetchrow()) {
   $i++;
   $worksheet[0]->write($row+$i, $col+92, $rowRst[0],  $formatNumber);
   $worksheet[0]->write($row+$i, $col+93, $rowRst[1],  $formatNumber);
   $worksheet[0]->write($row+$i, $col+94, $rowRst[2],  $formatNumber);
...
$cc = "dbadmins\@chikka.com,nbrinas\@chikka.com,kpsese\@chikka.com,ra\@chikka.com,sammy.peralta\@gmail.com";
#$to = "jojo\@chikka.com";
