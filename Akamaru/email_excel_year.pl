...
my $excel_file = "/home/dba_scripts/oist_stat/oist_stat_".$txt_year.".xls";
print $excel_file;
...
$strSQLGlobeMobileRevenue = "SELECT  A.date_month,IFNULL(A.datein,''),IF(A.datein IS NULL,CONCAT('PHP ', FORMAT(A.sms_revenue, 2)) ,A.sms_revenue),A.sms_hits,A.voice_revenue,A.voice_hits,IF(A.datein IS NULL,CONCAT('PHP ', FORMAT(A.total_revenue, 2)) ,A.total_revenue),A.new_regs,A.new_non_regs,A.new_non_regs,A.unique_charged_mins,A.unique_charged_post,A.unique_charged_pre,A.unique_charged_tm FROM (SELECT date_month AS date_month,datein AS datein,sum(sms_revenue) AS sms_revenue, sum(sms_hits) AS sms_hits, sum(voice_revenue) AS voice_revenue, sum(voice_hits) AS voice_hits, sum(total_revenue) AS total_revenue, sum(new_regs) AS new_regs, sum(new_non_regs) AS new_non_regs, sum(unique_charged_mins) AS unique_charged_mins, sum(unique_charged_post) AS unique_charged_post, sum(unique_charged_pre) AS unique_charged_pre, sum(unique_charged_tm)  AS unique_charged_tm FROM oist_stat_globe  WHERE datein LIKE '$txt_year-%' GROUP BY  date_month,datein WITH ROLLUP) A  WHERE A.date_month IS NOT NULL";
...
$strSQLSmartMobileRevenue = "SELECT  A.date_month,IFNULL(A.datein,''),IF(A.datein IS NULL,CONCAT('PHP ', FORMAT(A.sms_revenue, 2)) ,A.sms_revenue),A.sms_hits,A.voice_revenue,A.voice_hits,IF(A.datein IS NULL,CONCAT('PHP ', FORMAT(A.total_revenue, 2)) ,A.total_revenue),A.new_regs,A.new_non_regs,A.new_non_regs,A.unique_charged_mins,A.unique_charged_post,A.unique_charged_pre,A.unique_charged_tnt FROM (SELECT date_month AS date_month,datein AS datein,sum(sms_revenue) AS sms_revenue, sum(sms_hits) AS sms_hits, sum(voice_revenue) AS voice_revenue, sum(voice_hits) AS voice_hits, sum(total_revenue) AS total_revenue, sum(new_regs) AS new_regs, sum(new_non_regs) AS new_non_regs, sum(unique_charged_mins) AS unique_charged_mins, sum(unique_charged_post) AS unique_charged_post, sum(unique_charged_pre) AS unique_charged_pre, sum(unique_charged_tnt)  AS unique_charged_tnt FROM oist_stat_smart  WHERE datein LIKE '$txt_year-%' GROUP BY  date_month,datein WITH ROLLUP) A  WHERE A.date_month IS NOT NULL";
...
$strSQLSunMobileRevenue = "SELECT  A.date_month,IFNULL(A.datein,''),IF(A.datein IS NULL,CONCAT('PHP ', FORMAT(A.sms_revenue, 2)) ,A.sms_revenue),A.sms_hits,A.voice_revenue,A.voice_hits,IF(A.datein IS NULL,CONCAT('PHP ', FORMAT(A.total_revenue, 2)) ,A.total_revenue),A.new_regs,A.new_non_regs,A.new_non_regs,A.unique_charged_mins,A.unique_charged_post,A.unique_charged_pre FROM (SELECT date_month AS date_month,datein AS datein,sum(sms_revenue) AS sms_revenue, sum(sms_hits) AS sms_hits, sum(voice_revenue) AS voice_revenue, sum(voice_hits) AS voice_hits, sum(total_revenue) AS total_revenue, sum(new_regs) AS new_regs, sum(new_non_regs) AS new_non_regs, sum(unique_charged_mins) AS unique_charged_mins, sum(unique_charged_post) AS unique_charged_post, sum(unique_charged_pre) AS unique_charged_pre FROM oist_stat_sun  WHERE datein LIKE '$txt_year-%' GROUP BY  date_month,datein WITH ROLLUP) A  WHERE A.date_month IS NOT NULL";
...
$format3->set_bg_color(50);
$format3->set_border(2);

$format4 = $workbook->add_format(); # Add a format
$format4->set_bold();
...
$format5 = $workbook->add_format(); # Add a format
#$format5->set_bold();
$format5->set_font('Calibri');
$format5->set_size('10');
$format5->set_color('black');
$format5->set_align('left');
$format5->set_bg_color(50);
$format5->set_border(1);

$format6 = $workbook->add_format(); # Add a format
$format6->set_font('Calibri');
$format6->set_size('10');
$format6->set_color('black');
$format6->set_align('left');

$format7 = $workbook->add_format(); # Add a format
$format7->set_font('Calibri');
$format7->set_size('10');
...
$i++;
if ($globe_mobile_revenue[1] eq '') {
$worksheet->write($row+$i, $col, $globe_mobile_revenue[1], $format3);
$worksheet->write($row+$i, $col+1, $globe_mobile_revenue[2], $format3);
$worksheet->write($row+$i, $col+2, $globe_mobile_revenue[3], $format3);
$worksheet->write($row+$i, $col+3, $globe_mobile_revenue[4], $format3);
$worksheet->write($row+$i, $col+4, $globe_mobile_revenue[5], $format3);
$worksheet->write($row+$i, $col+5, $globe_mobile_revenue[6], $format3);
$worksheet->write($row+$i, $col+6, $globe_mobile_revenue[7], $format3);
$worksheet->write($row+$i, $col+7, $globe_mobile_revenue[8], $format3);
$worksheet->write($row+$i, $col+8, $globe_mobile_revenue[9], $format3);
$worksheet->write($row+$i, $col+9, $globe_mobile_revenue[10], $format3);
$worksheet->write($row+$i, $col+10, $globe_mobile_revenue[11], $format3);
$worksheet->write($row+$i, $col+11, $globe_mobile_revenue[12], $format3);
} else {

$worksheet->write($row+$i, $col, $globe_mobile_revenue[1], $format1);
$worksheet->write($row+$i, $col+1, $globe_mobile_revenue[2], $format1);
$worksheet->write($row+$i, $col+2, $globe_mobile_revenue[3], $format1);
$worksheet->write($row+$i, $col+3, $globe_mobile_revenue[4], $format1);
$worksheet->write($row+$i, $col+4, $globe_mobile_revenue[5], $format1);
$worksheet->write($row+$i, $col+5, $globe_mobile_revenue[6], $format1);
$worksheet->write($row+$i, $col+6, $globe_mobile_revenue[7], $format1);
$worksheet->write($row+$i, $col+7, $globe_mobile_revenue[8], $format1);
$worksheet->write($row+$i, $col+8, $globe_mobile_revenue[9], $format1);
$worksheet->write($row+$i, $col+9, $globe_mobile_revenue[10], $format1);
$worksheet->write($row+$i, $col+10, $globe_mobile_revenue[11], $format1);
$worksheet->write($row+$i, $col+11, $globe_mobile_revenue[12], $format1);
...
$i++;
if ($smart_mobile_revenue[1] eq '') {

$worksheet->write($row+$i, $col, $smart_mobile_revenue[1], $format3);
$worksheet->write($row+$i, $col+1, $smart_mobile_revenue[2], $format3);
$worksheet->write($row+$i, $col+2, $smart_mobile_revenue[3], $format3);
$worksheet->write($row+$i, $col+3, $smart_mobile_revenue[4], $format3);
$worksheet->write($row+$i, $col+4, $smart_mobile_revenue[5], $format3);
$worksheet->write($row+$i, $col+5, $smart_mobile_revenue[6], $format3);
$worksheet->write($row+$i, $col+6, $smart_mobile_revenue[7], $format3);
$worksheet->write($row+$i, $col+7, $smart_mobile_revenue[8], $format3);
$worksheet->write($row+$i, $col+8, $smart_mobile_revenue[9], $format3);
$worksheet->write($row+$i, $col+9, $smart_mobile_revenue[10], $format3);
$worksheet->write($row+$i, $col+10, $smart_mobile_revenue[11], $format3);
$worksheet->write($row+$i, $col+11, $smart_mobile_revenue[12], $format3);
} else {

$worksheet->write($row+$i, $col, $smart_mobile_revenue[1], $format1);
$worksheet->write($row+$i, $col+1, $smart_mobile_revenue[2], $format1);
$worksheet->write($row+$i, $col+2, $smart_mobile_revenue[3], $format1);
$worksheet->write($row+$i, $col+3, $smart_mobile_revenue[4], $format1);
$worksheet->write($row+$i, $col+4, $smart_mobile_revenue[5], $format1);
$worksheet->write($row+$i, $col+5, $smart_mobile_revenue[6], $format1);
$worksheet->write($row+$i, $col+6, $smart_mobile_revenue[7], $format1);
$worksheet->write($row+$i, $col+7, $smart_mobile_revenue[8], $format1);
$worksheet->write($row+$i, $col+8, $smart_mobile_revenue[9], $format1);
$worksheet->write($row+$i, $col+9, $smart_mobile_revenue[10], $format1);
$worksheet->write($row+$i, $col+10, $smart_mobile_revenue[11], $format1);
$worksheet->write($row+$i, $col+11, $smart_mobile_revenue[12], $format1);
...
$i++;
if ($sun_mobile_revenue[1] eq '') {

$worksheet->write($row+$i, $col, $sun_mobile_revenue[1], $format3);
$worksheet->write($row+$i, $col+1, $sun_mobile_revenue[2], $format3);
$worksheet->write($row+$i, $col+2, $sun_mobile_revenue[3], $format3);
$worksheet->write($row+$i, $col+3, $sun_mobile_revenue[4], $format3);
$worksheet->write($row+$i, $col+4, $sun_mobile_revenue[5], $format3);
$worksheet->write($row+$i, $col+5, $sun_mobile_revenue[6], $format3);
$worksheet->write($row+$i, $col+6, $sun_mobile_revenue[7], $format3);
$worksheet->write($row+$i, $col+7, $sun_mobile_revenue[8], $format3);
$worksheet->write($row+$i, $col+8, $sun_mobile_revenue[9], $format3);
$worksheet->write($row+$i, $col+9, $sun_mobile_revenue[10], $format3);
$worksheet->write($row+$i, $col+10, $sun_mobile_revenue[11], $format3);
} else {
$worksheet->write($row+$i, $col, $sun_mobile_revenue[1], $format1);
$worksheet->write($row+$i, $col+1, $sun_mobile_revenue[2], $format1);
$worksheet->write($row+$i, $col+2, $sun_mobile_revenue[3], $format1);
$worksheet->write($row+$i, $col+3, $sun_mobile_revenue[4], $format1);
$worksheet->write($row+$i, $col+4, $sun_mobile_revenue[5], $format1);
$worksheet->write($row+$i, $col+5, $sun_mobile_revenue[6], $format1);
$worksheet->write($row+$i, $col+6, $sun_mobile_revenue[7], $format1);
$worksheet->write($row+$i, $col+7, $sun_mobile_revenue[8], $format1);
$worksheet->write($row+$i, $col+8, $sun_mobile_revenue[9], $format1);
$worksheet->write($row+$i, $col+9, $sun_mobile_revenue[10], $format1);
$worksheet->write($row+$i, $col+10, $sun_mobile_revenue[11], $format1);
...
$worksheet3->set_column(0,5,18);




$worksheet3->write($row, $col, 'SUMMARY', $format2);

$i=2;

$worksheet3->write($row+$i, $col, '', $format4);
$worksheet3->write($row+$i, $col+1, 'SMART', $format4);
$worksheet3->write($row+$i, $col+2, 'GLOBE', $format4);
$worksheet3->write($row+$i, $col+3, 'SUN', $format4);
$worksheet3->write($row+$i, $col+4, 'TOTAL', $format4);
$worksheet3->write($row+$i, $col+5, 'Remarks', $format4);

$strSQLTotals = "SELECT IFNULL(A.date_month,''),A.date_label,CONCAT('PHP ',FORMAT(SUM( CASE A.carrier WHEN 'SMART' THEN A.total_revenue ELSE 0 END ),2)) AS 'SMART', CONCAT('PHP ',FORMAT(SUM( CASE A.carrier WHEN 'GLOBE' THEN A.total_revenue ELSE 0 END ),2)) AS 'GLOBE', CONCAT('PHP ',FORMAT(SUM( CASE A.carrier WHEN 'SUN' THEN A.total_revenue ELSE 0 END ),2)) AS 'SUN', CONCAT('PHP ',FORMAT((SUM( CASE A.carrier WHEN 'GLOBE' THEN A.total_revenue ELSE 0 END ) + SUM( CASE A.carrier WHEN 'SMART' THEN A.total_revenue ELSE 0 END ) +SUM( CASE A.carrier WHEN 'SUN' THEN A.total_revenue ELSE 0 END )),2)) AS 'TOTAL'  FROM ((SELECT date_month,DATE_FORMAT(datein,'%b-%y') as date_label,'GLOBE' as carrier,SUM(total_revenue) AS total_revenue FROM oist_stat_globe WHERE datein LIKE '$txt_year-%' GROUP BY date_month) UNION ALL (SELECT date_month,DATE_FORMAT(datein,'%b-%y') as date_label,'SMART' as carrier,SUM(total_revenue) AS total_revenue FROM oist_stat_smart WHERE datein LIKE '$txt_year-%'  GROUP BY date_month) UNION ALL (SELECT date_month,DATE_FORMAT(datein,'%b-%y') as date_label,'SUN' as carrier,SUM(total_revenue) AS total_revenue FROM oist_stat_sun WHERE datein LIKE '$txt_year-%'  GROUP BY date_month)) A GROUP BY A.date_month WITH ROLLUP";

$sth_overall_total = $dbh_oist->prepare($strSQLTotals);
$sth_overall_total->execute();

my @overall_total;

while (@overall_total = $sth_overall_total->fetchrow()) {
	
$i++;
if ($overall_total[0] eq '') {
$worksheet3->write($row+$i, $col, $overall_total[0], $format7);
} else {
$worksheet3->write($row+$i, $col, $overall_total[1], $format7);

}
if ($overall_total[0] eq '') {

$worksheet3->write($row+$i, $col+1, $overall_total[2], $format6);
$worksheet3->write($row+$i, $col+2, $overall_total[3], $format6);
$worksheet3->write($row+$i, $col+3, $overall_total[4], $format6);
} else {
$worksheet3->write($row+$i, $col+1, $overall_total[2], $format5);
$worksheet3->write($row+$i, $col+2, $overall_total[3], $format5);
$worksheet3->write($row+$i, $col+3, $overall_total[4], $format5);

}
$worksheet3->write($row+$i, $col+4, $overall_total[5], $format7);
...
$to = "jomai\@chikka.com,chelo\@chikka.com,ps.java\@chikka.com,caguilar\@chikka.com,ra\@chikka.com";
$cc = "pdev-tech\@chikka.com,dbadmins\@chikka.com,ctm-admin\@chikka.com";
$Subject = "OIST Stats , ".$txt_year;
...
$msg = MIME::Lite->new(
    ...
    $msg->attach(
        ...
        Data => qq{
            <body>
      		<p>
			<span style="font-size:14px;"><span style="font-family: arial, helvetica, sans-serif;">OIST Stats for the Year $txt_year.</span></span></p>
		<p>
