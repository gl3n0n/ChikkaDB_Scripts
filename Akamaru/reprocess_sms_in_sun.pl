...
my $charging = common::chargingOperator('SUN');
#
...
my $dbh_csg = DB::DBconnect(myconstants::CSG_V3_DB_NAGATO,myconstants::CSG_V3_HOST_NAGATO,myconstants::CSG_V3_USER,myconstants::CSG_V3_PASSWORD);
...
my $sth_globe_post = "";
my $sth_globe_tm  = "";
#$qry_sms_mobile = "SELECT gsm_num FROM sms_in WHERE datein ='$current_date' AND left(gsm_num,5) IN (".$prefix_mobile.")  GROUP BY 1";

my $qry_sms_all = "SELECT GROUP_CONCAT(pattern SEPARATOR  '\$\|^') FROM mobile_pattern where operator='SUN'";
my $qry_sms_post = "SELECT GROUP_CONCAT(pattern SEPARATOR  '\$\|^') FROM mobile_pattern where operator='SUN' AND sim_type='POSTPAID'";
...
my $qry_globe_all_sms = "SELECT COUNT(1) FROM sms_in WHERE datein ='$current_date' AND  access_code LIKE '2009%'";

my $qry_globe_all_unique = "SELECT COUNT(DISTINCT(gsm_num)) FROM sms_in WHERE datein ='$current_date' AND  gsm_num REGEXP '^".$globe_all_pattern[0]."\$'";
my $qry_globe_postpaid_unique = "SELECT COUNT(DISTINCT(gsm_num)) FROM sms_in WHERE datein ='$current_date' AND  gsm_num REGEXP '^".$globe_post_pattern[0]."\$'";
#my $qry_globe_tm_unique = "SELECT COUNT(DISTINCT(gsm_num)) FROM sms_in WHERE datein ='$current_date' AND  gsm_num REGEXP '^".$globe_tm_pattern[0]."\$'";
...
my $sth_qry_insert  = "";
my $qry_insert = "REPLACE INTO oist_stat_sun (datein,sms_revenue,sms_hits,unique_charged_mins,unique_charged_post,unique_charged_pre) VALUES ('$current_date',$globe_cnt_all_sms[0]*$charging,$globe_cnt_all_sms[0],$globe_cnt_all_unique[0],$globe_cnt_postpaid_unique[0],$globe_cnt_all_unique[0]-$globe_cnt_postpaid_unique[0])";
...
my $strUpdateTotalRevenue = "UPDATE oist_stat_sun SET total_revenue=sms_revenue+voice_revenue WHERE datein ='$current_date'";
