...
my $file_temp = "/var/log/oist/oist_".$event_type."_".$tbl_date;
 ...
 if($_ =~ m/(\d+-\d+-\d+)\s+(\d+:\d+):(.*)\s+\w+\s+\w+\s+(.*)\s+(\w+)\s+(.*)\s+(.*)\s+(.*)\s+(\w+)/g)  
  {
      ...
      print FILE "$1||$2||$3||$4||$5||$6||$7||$8||$9\n";
     # $sth1 = $dbh_oist->prepare($insert_data);
     # $sth1 -> execute();        
   }
}

my $qry_load_file = "LOAD DATA INFILE '$file_temp' INTO TABLE oist_log.oist_".$event_type."_".$tbl_date." FIELDS TERMINATED BY '\|\|' LINES TERMINATED BY '\\n'";
$sth1 = $dbh_oist->prepare($qry_load_file);
$sth1 -> execute();        
